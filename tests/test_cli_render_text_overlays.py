"""Tests for the `render-text-overlays` CLI command (src/cli.py
cmd_render_text_overlays). Same isolated_db / _create_registered_project
pattern as every other CLI-level test in this repo. The core pipeline
(src.core.text_overlay_render.render_text_overlays) is always mocked in
...
GENERATION_MODULE = "src.core.text_overlay_render"
...
def _mock_pipeline_success(monkeypatch):
    from src.core.text_overlay_render import TextOverlayRenderResult

    def _fake_render(project_id, manifest_path, output_path):
        Path(output_path).write_bytes(b"fake-overlay-mp4-bytes")
        return TextOverlayRenderResult(
            project_id=project_id,
            output_path=Path(output_path),
            overlay_count=2,
            measured_duration_seconds=3.5,
            artifact_id="overlay-render-final",
        )

    monkeypatch.setattr(f"{GENERATION_MODULE}.render_text_overlays", _fake_render)
...
def test_parser_registers_render_text_overlays():
    parser = cli.build_parser()
    args = parser.parse_args(
        ["render-text-overlays", "proj-123", "--manifest", "m.json", "--output", "out.mp4"]
    )
    assert args.func is cli.cmd_render_text_overlays
...
def test_project_id_is_positional():
    ...
    with pytest.raises(SystemExit):
        parser.parse_args(["render-text-overlays", "--manifest", "m.json", "--output", "out.mp4"])


def test_manifest_is_required():
    parser = cli.build_parser()
    with pytest.raises(SystemExit):
        parser.parse_args(["render-text-overlays", "proj-123", "--output", "out.mp4"])


def test_output_is_required():
    parser = cli.build_parser()
    with pytest.raises(SystemExit):
        parser.parse_args(["render-text-overlays", "proj-123", "--manifest", "m.json"])
...
def test_cli_success_text_mode(isolated_db, tmp_path, capsys, monkeypatch):
    ...
    output_path = tmp_path / "overlay-final.mp4"
    rc = cli.cmd_render_text_overlays(_args(project_id, manifest_path, output_path))
    out = capsys.readouterr().out

    assert rc == 0
    assert "render-text-overlays: OK" in out
    assert f"project_id: {project_id}" in out
    assert f"output: {output_path}" in out
    assert "overlay_count: 2" in out
    assert "measured_duration_seconds: 3.5" in out
    assert "artifact_id: overlay-render-final" in out
...
def test_cli_success_json_mode(isolated_db, tmp_path, capsys, monkeypatch):
    ...
    manifest_path = project_dir / "manifest.json"
    output_path = tmp_path / "overlay-final.mp4"
    rc = cli.cmd_render_text_overlays(_args(project_id, manifest_path, output_path, out_format="json"))
    out = capsys.readouterr().out
...
def test_cli_calls_production_entry_point(isolated_db, tmp_path, capsys, monkeypatch):
    from src.core.text_overlay_render import TextOverlayRenderResult
    ...
    cli.cmd_render_text_overlays(_args(project_id, manifest_path, output_path))
    capsys.readouterr()
...
def test_cli_domain_failure_written_to_stderr(isolated_db, tmp_path, capsys, monkeypatch):
    from src.core.text_overlay_render import ProjectNotFoundError

    def _fail(project_id, manifest_path, output_path):
        raise ProjectNotFoundError(f"unknown project_id {project_id!r}")

    monkeypatch.setattr(f"{GENERATION_MODULE}.render_text_overlays", _fail)
    ...
    assert "render-text-overlays: FAILED" in err
...
def test_cli_domain_failure_returns_nonzero(isolated_db, tmp_path, capsys, monkeypatch):
    from src.core.text_overlay_render import NoOverlaysToRenderError

    monkeypatch.setattr(
        f"{GENERATION_MODULE}.render_text_overlays",
        lambda *a, **k: (_ for _ in ()).throw(NoOverlaysToRenderError("boom")),
    )
...
def test_cli_domain_failure_message_does_not_expose_secrets(isolated_db, tmp_path, capsys, monkeypatch):
    ...
    def _fail(project_id, manifest_path, output_path):
        raise TextOverlayRenderError(sentinel)
...
def test_cli_text_domain_error_written_only_to_stderr(isolated_db, tmp_path, capsys, monkeypatch):
    ...
    rc = cli.cmd_render_text_overlays(_args(project_id, manifest_path, output_path, out_format="text"))
    out, err = capsys.readouterr()
...
def test_cli_invalid_format_rejects(isolated_db, tmp_path, capsys):
    ...
    rc = cli.cmd_render_text_overlays(_args("proj-x", manifest_path, tmp_path / "out.mp4", out_format="xml"))
    err = capsys.readouterr().err
...
def test_cli_opens_no_connection_of_its_own(isolated_db, tmp_path, monkeypatch):
    ...
    source = inspect.getsource(cli.cmd_render_text_overlays)
