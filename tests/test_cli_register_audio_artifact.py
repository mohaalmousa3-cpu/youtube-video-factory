"""Tests for the `register-audio-artifact` CLI command (src/cli.py
cmd_register_audio_artifact). Same isolated_db / _create_registered_project
pattern as tests/test_cli_verify_and_advance.py. Uses the REAL local
...
def _ns(**kwargs) -> argparse.Namespace:
    defaults = dict(project_id="proj-123", scene_id="scene-01", file="audio.wav", format="text")
...
def test_register_audio_artifact_parser_registration():
    parser = cli.build_parser()
    args = parser.parse_args(
        ["register-audio-artifact", "proj-123", "--scene-id", "scene-01", "--file", "audio.wav"]
    )
    assert args.func is cli.cmd_register_audio_artifact
...
def test_register_audio_artifact_requires_scene_id_and_file():
    parser = cli.build_parser()
    with pytest.raises(SystemExit):
        parser.parse_args(["register-audio-artifact", "proj-123", "--file", "audio.wav"])
    with pytest.raises(SystemExit):
        parser.parse_args(["register-audio-artifact", "proj-123", "--scene-id", "scene-01"])
...
def test_cli_success_registers_artifact_and_copies_file(isolated_db, tmp_path, capsys):
    ...
    rc = cli.cmd_register_audio_artifact(
        argparse.Namespace(project_id=project_id, scene_id="scene-01", file=str(source), format="text")
    )
    out = capsys.readouterr().out

    assert rc == 0
    assert "register-audio-artifact: OK" in out
    assert (project_dir / "audio" / "scene-01.wav").exists()
...
def test_cli_success_json_output_shape(isolated_db, tmp_path, capsys):
    ...
    assert payload["artifact_id"] == "audio-scene-01"
    assert payload["relative_path"] == "audio/scene-01.wav"
    assert payload["artifact"]["kind"] == "audio"
...
def test_cli_duplicate_identical_run_is_idempotent(isolated_db, tmp_path, capsys):
    ...
    rc1 = cli.cmd_register_audio_artifact(ns)
    capsys.readouterr()
    rc2 = cli.cmd_register_audio_artifact(ns)
    ...
    conn = get_connection()
    try:
        artifacts = list_artifacts_by_project(conn, project_id, kind="audio")
    finally:
        conn.close()
...
def test_cli_duplicate_different_content_is_rejected(isolated_db, tmp_path, capsys):
    ...
    cli.cmd_register_audio_artifact(
    ...
    _real_wav(source_b, duration_seconds=2.0)
    rc = cli.cmd_register_audio_artifact(
...
def test_cli_missing_database_has_zero_filesystem_side_effects(isolated_db, capsys):
    ...
    rc = cli.cmd_register_audio_artifact(
        argparse.Namespace(project_id="proj-x", scene_id="scene-01", file="audio.wav", format="text")
    )
    err = capsys.readouterr().err
...
def test_cli_invalid_format_returns_error(isolated_db, capsys):
    rc = cli.cmd_register_audio_artifact(_ns(format="yaml"))
    err = capsys.readouterr().err
    assert rc == 1
...
def test_cli_end_to_end_register_then_verify_and_advance(isolated_db, tmp_path, capsys):
    ...
    source = tmp_path / "source.wav"
    _real_wav(source)
    rc_register = cli.cmd_register_audio_artifact(
        argparse.Namespace(project_id=project_id, scene_id="scene-01", file=str(source), format="text")
    )
    capsys.readouterr()
    ...
    rc_advance = cli.cmd_verify_and_advance(
        argparse.Namespace(project_id=project_id, to="audio_ready", reason="smoke test", format="text")
    )
