"""Tests for the `register-animation-artifact` CLI command (src/cli.py
cmd_register_animation_artifact). Same isolated_db / _create_registered_project
pattern as tests/test_cli_register_audio_artifact.py. Uses the REAL local
ffmpeg/ffprobe binary (no mocking) except where a test is specifically
about a database failure."""
...
def _advance_to_animation_pending(project_id: str) -> None:
...
def _ns(**kwargs) -> argparse.Namespace:
    defaults = dict(project_id="proj-123", scene_id="scene-01", file="video.mp4", format="text")
...
def test_register_animation_artifact_parser_registration():
    parser = cli.build_parser()
    args = parser.parse_args(
        ["register-animation-artifact", "proj-123", "--scene-id", "scene-01", "--file", "video.mp4"]
    )
    assert args.func is cli.cmd_register_animation_artifact
...
def test_register_animation_artifact_requires_scene_id_and_file():
    parser = cli.build_parser()
    with pytest.raises(SystemExit):
        parser.parse_args(["register-animation-artifact", "proj-123", "--file", "video.mp4"])
    with pytest.raises(SystemExit):
        parser.parse_args(["register-animation-artifact", "proj-123", "--scene-id", "scene-01"])
...
def test_cli_success_registers_artifact_and_copies_file(isolated_db, tmp_path, capsys):
    ...
    rc = cli.cmd_register_animation_artifact(
        argparse.Namespace(project_id=project_id, scene_id="scene-01", file=str(source), format="text")
    )
    out = capsys.readouterr().out

    assert rc == 0
    assert "register-animation-artifact: OK" in out
    assert (project_dir / "animation" / "scene-01.mp4").exists()
...
def test_cli_success_json_output_shape(isolated_db, tmp_path, capsys):
    ...
    assert payload["artifact_id"] == "animation-scene-01"
    assert payload["relative_path"] == "animation/scene-01.mp4"
    assert payload["duration_seconds"] == pytest.approx(1.5, abs=0.3)
    assert payload["artifact"]["kind"] == "animation"
...
def test_cli_duplicate_identical_run_is_idempotent(isolated_db, tmp_path, capsys):
    ...
    rc1 = cli.cmd_register_animation_artifact(ns)
    capsys.readouterr()
    rc2 = cli.cmd_register_animation_artifact(ns)
    ...
    conn = get_connection()
    try:
        artifacts = list_artifacts_by_project(conn, project_id, kind="animation")
    finally:
        conn.close()
...
def test_cli_duplicate_different_content_is_rejected(isolated_db, tmp_path, capsys):
    ...
    cli.cmd_register_animation_artifact(
...
def test_cli_audio_only_video_returns_error(isolated_db, tmp_path, capsys):
    ...
    rc = cli.cmd_register_animation_artifact(
        argparse.Namespace(project_id=project_id, scene_id="scene-01", file=str(audio_only), format="text")
    )
    out = capsys.readouterr().out
...
def test_cli_missing_database_has_zero_filesystem_side_effects(isolated_db, capsys):
    ...
    rc = cli.cmd_register_animation_artifact(
        argparse.Namespace(project_id="proj-x", scene_id="scene-01", file="video.mp4", format="text")
    )
    err = capsys.readouterr().err
...
def test_cli_invalid_format_returns_error(isolated_db, capsys):
    rc = cli.cmd_register_animation_artifact(_ns(format="yaml"))
    err = capsys.readouterr().err
    assert rc == 1
...
def test_cli_end_to_end_register_then_verify_and_advance(isolated_db, tmp_path, capsys):
    project_id, project_dir = _create_registered_project(tmp_path / "projects", scene_ids=("scene-01",))
    _advance_to_animation_pending(project_id)

    source = tmp_path / "source.mp4"
    _real_mp4(source)
    rc_register = cli.cmd_register_animation_artifact(
        argparse.Namespace(project_id=project_id, scene_id="scene-01", file=str(source), format="text")
    )
    capsys.readouterr()
    ...
    rc_advance = cli.cmd_verify_and_advance(
        argparse.Namespace(project_id=project_id, to="animation_ready", reason="smoke test", format="text")
    )
