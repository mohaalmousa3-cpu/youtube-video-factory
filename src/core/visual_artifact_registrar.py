"""Phase 2E: registration-first Visual Artifact Registration Pipeline.

Registers a single, already-produced local PNG image file as one scene's
"visual" artifact, so a project can later satisfy Phase 2C's
verify-and-advance(..., to_stage="visuals_ready", ...) requirement of one
verified visual artifact per scene. This module never generates an image
(no Qwen-Image, no provider call), never mutates the manifest or
...
It is also stage-agnostic: it never checks or cares what current_stage the
project is in.

The canonical identity for a project's scene is fixed and deterministic —
there is no alternate-take support in Phase 2E, matching Phase 2D's own
audio_artifact_registrar.py design:

    artifact_id:    visual-<scene_id>
    kind:           visual
    scene_id:       <scene_id>
    relative_path:  visuals/<scene_id>.png

Local filesystem and local SQLite only, plus one local, read-only decode
of the source file via Pillow (already a pinned dependency, used elsewhere
in this repo by src/render/character_rig.py) to validate it is a genuine,
fully-decodable PNG before ever copying it — PNG only: a source file that
Pillow can decode as some OTHER format (e.g. a real JPEG) is rejected just
as firmly as one it cannot decode at all, since every image this project's
pipeline produces or consumes is PNG (src/providers/image_qwen.py,
src/render/character_rig.py). No LLM, TTS, image generation, Flow, Veo,
renderer *invocation*, YouTube, network/API, or payment call anywhere in
this module. No Qwen-specific size/aspect-ratio restriction is enforced —
this registrar accepts any decodable PNG of any dimensions.

Write paths, exactly three — identical shape to audio_artifact_registrar.py:
  1. mkdir <project_dir>/visuals/ (only once every validation has passed)
  2. copy the source file to <project_dir>/visuals/<scene_id>.png (only for
     a genuinely fresh registration — never when the destination already
     has byte-identical content)
  3. register_artifact() — exactly one INSERT, and only after (1) and (2)
     (if needed) have already succeeded

Every ordinary rejection (unknown scene, invalid source file, an
undecodable or non-PNG image, an existing record/file with different
content, a documented register_artifact() failure —
ArtifactAlreadyExistsError, DuplicateArtifactRegistrationError,
ArtifactProjectNotFoundError, ArtifactRegistrationError, or any
sqlite3.Error, ...) is reported via the returned
VisualArtifactRegistrationResult with ok=False — this module does not
raise for those. It raises VisualArtifactRegistrationError only for a
...
src/core/verified_transition_service.py and audio_artifact_registrar.py
...
Deliberately NOT shared with audio_artifact_registrar.py in this phase:
_sha256_of_file() and the fresh-copy cleanup helper below are intentional,
small, self-contained duplicates of that module's own private helpers, not
imports from it — a scoped choice for this phase, not an oversight; a
shared-helper extraction is left for a later, deliberate maintenance
refactor."""
...
class VisualArtifactRegistrationError(Exception):
    ...
    scene, invalid source file, an undecodable or non-PNG image, a
    conflicting existing record/file, ...) are reported via a returned
    VisualArtifactRegistrationResult instead; see this module's
    docstring."""


@dataclass(frozen=True)
class VisualArtifactRegistrationResult:
    """The one typed result register_visual_artifact() always returns,
...
def _validate_png(source_file: Path) -> tuple[tuple[int, int, str] | None, str | None]:
    """Decode `source_file` with Pillow and require it to be a genuine,
    fully-decodable PNG — not merely a file with a '.png' extension. A
    file Pillow cannot identify at all, one that fails to fully decode
    (e.g. truncated mid-stream), or one that decodes as some OTHER real
    format (e.g. an actual JPEG) is rejected. Returns
    ((width, height, "PNG"), None) on success or (None, error_message) on
    any failure."""
    try:
        ...
        return None, f"could not decode image file at {source_file}: {exc}"
    if image_format != "PNG":
        return None, (
            f"source file at {source_file} is not a PNG (Pillow identified it as "
            f"{image_format!r}) — only PNG is accepted"
        )
...
def register_visual_artifact(
    ...
    *,
...
) -> VisualArtifactRegistrationResult:
    """Register `source_file` as project/scene_id's canonical "visual"
    ...
    if manifest.project_id != project.project_id:
        raise VisualArtifactRegistrationError(
    ...
    artifact_id = f"visual-{scene_id}"
    relative_path = f"visuals/{scene_id}.png"
    ...
    def _result(
        *,
        ...
        return VisualArtifactRegistrationResult(
    ...
    # DB write. A mismatch is rejected outright; Phase 2E never overwrites
    # an existing registration.
    existing_matches = [a for a in list_artifacts_by_scene(conn, project.project_id, scene_id) if a.kind == "visual"]
    if existing_matches:
        ...
        return _result(
            ok=False,
            reasons=(
                f"a visual artifact is already registered for scene {scene_id!r} "
                f"({existing.artifact_id!r}) with different content — Phase 2E never overwrites "
        ...
        if dest_checksum != source_checksum or dest_size != source_size:
            return _result(
                ...
                reasons=(
                    ...
                    "Phase 2E never overwrites an existing file",
                ),
            )
    ...
    image_info, image_error = _validate_png(source_file)
    if image_error is not None:
        return _result(ok=False, reasons=(image_error,))
    width, height, image_format = image_info

    if final_checksum is None:
        # Fresh path: nothing exists yet at `destination` — create
        # visuals/ only now, after every validation (including PNG
        # decode validation) has already passed.
    ...
    record = ArtifactRecord(
        ...
        created_at=when,
        metadata={"width": width, "height": height, "format": image_format, "source": "external"},
    )
    ...
    try:
        ...
        return _result(ok=False, width=width, height=height, image_format=image_format, reasons=(str(exc),))
    except Exception:
